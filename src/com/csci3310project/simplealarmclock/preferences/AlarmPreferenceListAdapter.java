...
import com.csci3310project.simplealarmclock.preferences.AlarmPreference.Type;
...
public class AlarmPreferenceListAdapter extends BaseAdapter {
	...
	private final String[] alarmToneType = {"Ringtone", "Music"};
	...
	public AlarmPreferenceListAdapter(Context context, Alarm alarm) {
		...
		musicPaths = new String[musicCursor.getCount()+1];
		musicPaths[0] = "";
		musicTitles= new String[musicCursor.getCount()+1];
		musicTitles[0] = "Silent";
		
		if (musicCursor.moveToFirst()){
			do{
				musicPaths[musicCursor.getPosition()+1] =
						musicCursor.getString(musicCursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA));
				musicTitles[musicCursor.getPosition()+1] =
						musicCursor.getString(musicCursor.getColumnIndexOrThrow
...
//		
		...
		for(AlarmPreference preference : preferences){
			switch(preference.getKey()){
				...
				case ALARM_TONE_TYPE:
					alarm.setToneType(Alarm.ToneType.valueOf((String) preference.getValue()));
				case ALARM_TONE:
					if (alarm.getToneType() == ToneType.RINGTONE)
						alarm.setAlarmTonePath((String) preference.getValue());
					else
						alarm.setMusicPath((String) preference.getValue());
					break;
				case ALARM_VIBRATE:
	...
	public void setMathAlarm(Alarm alarm) {
		...
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE_TYPE,"Sound Type",
				alarm.getToneType().toString(), alarmToneType, alarm.getToneType(), Type.LIST));
		...
		if (alarm.getToneType() == ToneType.RINGTONE){
			...
			if(alarmTone instanceof Ringtone && !alarm.getAlarmTonePath().equalsIgnoreCase("")){
				preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE, "Ringtone",
						alarmTone.getTitle(getContext()),alarmTones, alarm.getAlarmTonePath(), Type.LIST));
			}else{
				preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE, "Ringtone",
						getAlarmTones()[0],alarmTones, null, Type.LIST));
			}
		} else{
			Uri musicUri = Uri.parse(alarm.getMusicPath());
			File music = new File(musicUri.getPath());
			
			if(!alarm.getMusicPath().equalsIgnoreCase("")){
				preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE, "Music",
						music.getName(),musicTitles, alarm.getMusicPath(), Type.LIST));
			}else{
				preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE, "Music",
						getMusicTitles()[0], musicTitles, null, Type.LIST));
			}
		}
